...
def generate_tts(text: str, lang: str) -> str:
    """
    Synthesise verification prompt via two-layer TTS pipeline.

    Layer 1 (Primary): Sarvam TTS (bulbul:v2) — best for Indian languages.
    Layer 2 (Fallback): Edge TTS with +50% volume boost.

    Always returns a path string. The file will be a valid MP3 if TTS
    succeeded, or missing/empty if both layers failed (caller must handle
    the missing-file case gracefully before playback).
    """
    ...
    if audio_bytes and len(audio_bytes) > 100:
        with open(output_path, "wb") as f:
            f.write(audio_bytes)
        logger.info("[SARVAM TTS] Saved %d bytes to %s", len(audio_bytes), output_path)
        return output_path

    # ── Layer 2: Edge TTS (fallback) ─────────────────────────────────────────
    ...
    # Primary Edge TTS voice failed — try the language-agnostic English fallback
    if voice != TTS_FALLBACK_VOICE:
        logger.warning("[EDGE TTS] Primary voice failed — retrying with fallback voice %s.", TTS_FALLBACK_VOICE)
        if _edge_tts_attempt(TTS_FALLBACK_VOICE):
            return output_path

    logger.error("[TTS] Both Sarvam and Edge TTS failed — no audio will be played.")
    return output_path
