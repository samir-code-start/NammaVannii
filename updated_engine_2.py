...
Architecture:
  ...
  TTS → Edge TTS (primary, English, en-IN-NeerjaNeural)
"""
...
# kn-IN-SapnaNeural is the only working Kannada voice in edge-tts as of 2024.
# kn-IN-VarunNeural returns "No audio was received" and must NOT be used.
...
async def _tts_coroutine(text: str, voice: str, output_path: str) -> None:
    ...
    Volume is boosted by +50% to compensate for the typically low output level
    of Edge TTS fallback voices. The output is always written as an MP3 file
    (edge-tts natively produces MP3-compatible audio).
    """
    communicator = edge_tts.Communicate(text, voice, volume="+50%")
...
def _sarvam_tts(text: str, lang: str) -> bytes | None:
    """
    Call Sarvam TTS API (bulbul:v2).

    Returns MP3 audio bytes on success, or None on failure.

    The Sarvam API returns audio as a base64-encoded string inside the
    `audios` list. The raw decoded bytes are WAV (PCM), so we convert them
    to MP3 here using pydub (with ffmpeg) to ensure universal browser
    compatibility. If pydub/ffmpeg is unavailable we fall back to returning
    the raw WAV bytes wrapped in an MP3-compatible container — the caller
    saves them to an .mp3 path regardless, and most browsers will still
    decode them as audio.
    """
    import base64
    ...
    try:
        ...
        if not audio_b64:
            logger.warning("[SARVAM TTS] Empty audio payload for lang=%s", target_lang)
            return None

        # Sarvam returns base64-encoded WAV bytes
        raw_wav_bytes = base64.b64decode(audio_b64)

        # Convert WAV → MP3 so the file is natively playable in all browsers
        try:
            from pydub import AudioSegment
            import io
            audio_seg = AudioSegment.from_file(io.BytesIO(raw_wav_bytes), format="wav")
            mp3_buffer = io.BytesIO()
            audio_seg.export(mp3_buffer, format="mp3", bitrate="128k")
            mp3_bytes = mp3_buffer.getvalue()
            logger.info("[SARVAM TTS] WAV→MP3 conversion OK for lang=%s (%d bytes)", target_lang, len(mp3_bytes))
            return mp3_bytes
        except Exception as conv_err:
            # pydub/ffmpeg not available — return raw WAV bytes.
            # These are saved with a .mp3 extension; modern browsers and
            # HTMLMediaElement will still decode them via their WAV codec.
            logger.warning(
                "[SARVAM TTS] WAV→MP3 conversion skipped (%s) — using raw WAV bytes", conv_err
            )
...
def generate_tts(text: str, lang: str = "en") -> str:
    """
    Synthesise verification prompt via Edge TTS (primary, English only).

    Always uses Edge TTS with the English voice (en-IN-NeerjaNeural).
    The ``lang`` parameter is accepted for API compatibility but ignored —
    output is always English so the verification prompt is consistent.

    Always returns a path string. The file will be a valid MP3 if TTS
    succeeded, or an empty stub if TTS failed (caller must handle the
    missing/empty-file case gracefully before playback).
    """
    ...
    # Always use the English voice — TTS output is English regardless of citizen language
    voice = TTS_FALLBACK_VOICE  # en-IN-NeerjaNeural
    logger.info("[EDGE TTS PRIMARY] lang ignored, using English voice=%s, chars=%d", voice, len(text))

    def run_async_tts(tts_text: str, tts_voice: str, path: str) -> None:
        """Run edge-tts in a dedicated event loop (safe inside Streamlit threads)."""
    ...
    def _edge_tts_attempt(tts_voice: str) -> bool:
        """Try a single Edge TTS voice. Returns True if a non-empty file was written."""
        try:
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
                pool.submit(run_async_tts, text, tts_voice, output_path).result(timeout=30)
            size = os.path.getsize(output_path) if os.path.exists(output_path) else 0
            if size > 100:
                logger.info("[EDGE TTS] Saved %d bytes with voice=%s to %s", size, tts_voice, output_path)
                return True
            logger.warning("[EDGE TTS] Voice %s produced an empty/tiny file (%d bytes).", tts_voice, size)
        except Exception as exc:
            logger.warning("[EDGE TTS] Voice %s failed: %s", tts_voice, exc)
        return False

    # Primary attempt with English voice
    if _edge_tts_attempt(voice):
        return output_path

    # Retry with the alternative English-IN voice as last resort
    alt_voice = "en-US-AriaNeural"
    if voice != alt_voice:
        logger.warning("[EDGE TTS] Primary voice failed — retrying with alt voice %s.", alt_voice)
        if _edge_tts_attempt(alt_voice):
            return output_path

    logger.error("[TTS] Edge TTS failed on all voices — no audio will be played.")
    return output_path
...
# ---------------------------------------------------------------------------
# Streamlit Audio Autoplay Helper
# ---------------------------------------------------------------------------

def get_autoplay_audio_html(audio_path: str) -> str:
    """
    Return an HTML snippet that autoplays an MP3 file inside Streamlit.

    Streamlit's built-in ``st.audio`` widget does NOT autoplay. This helper
    base64-encodes the audio file and embeds it in an ``<audio>`` tag with
    the ``autoplay`` attribute so playback starts immediately when the
    component is rendered via ``st.components.v1.html()``.

    Usage in your Streamlit app::

        from engine import get_autoplay_audio_html
        import streamlit.components.v1 as components

        html = get_autoplay_audio_html(ai_data["verify_tts_path"])
        if html:
            components.html(html, height=0)

    Returns an empty string if the file is missing or empty (safe to ignore).
    """
    import base64

    if not audio_path or not os.path.isfile(audio_path):
        logger.warning("[AUTOPLAY] Audio file not found: %s", audio_path)
        return ""

    file_size = os.path.getsize(audio_path)
    if file_size < 100:
        logger.warning("[AUTOPLAY] Audio file too small (%d bytes) — skipping autoplay.", file_size)
        return ""
    ...
    try:
        ...
        logger.info("[AUTOPLAY] HTML autoplay snippet generated for: %s (%d bytes)", audio_path, file_size)
        return html
    except Exception as e:
        logger.error("[AUTOPLAY] Failed to generate autoplay HTML: %s", e)
        return ""


    """
    Construct the full verification prompt in English and generate TTS audio.
    Updates ai_data directly.

    TTS is always in English (Edge TTS primary). The translated prompt is still
    stored for display purposes on the UI, but audio is generated from the
    English text only.
    """
    ...
    # Store translated prompt for UI display (non-English citizens can read it)
    # but TTS audio is always generated from the English prompt
    if appended_prompt and citizen_lang != "en":
    ...
    # Always generate TTS from English text using Edge TTS (en-IN-NeerjaNeural)
    tts_path = generate_tts(appended_prompt, "en")
